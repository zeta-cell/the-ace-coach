...
const TrainingBlocksPanel = ({ onApplyBlock, onSaveAsBlock, modules, isOpen = true, onClose }: TrainingBlocksPanelProps) => {
  ...
  const renderBlockList = () => {
    ...
    return (
      <div className="space-y-1 max-h-[calc(100vh-320px)] overflow-y-auto pr-1 scrollbar-none">
                          ...
                          <div key={block.id}
                            ...
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <div className="flex items-center gap-0.5">
                                {!block.is_system && (
                                  <button onClick={(e) => handleDeleteBlock(block.id, e)}
        ...
        {goals.size === 0 && <div className="text-center py-6"><p className="text-xs font-body text-muted-foreground">No blocks found</p></div>}
      </div>
    );
  };
      ...
      {isOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background/40 z-40 lg:hidden" onClick={onClose} />
          <motion.div
