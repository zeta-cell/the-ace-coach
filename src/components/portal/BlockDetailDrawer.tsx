...
import {
  ...
  Heart, Award, MapPin, Play, ExternalLink, UserPlus, BookOpen, Zap,
} from "lucide-react";
import { Sheet, SheetContent } from "@/components/ui/sheet";
import BlockAssignCoachFlow from "./BlockAssignCoachFlow";
  ...
  return (
    <Sheet open={!!block} onOpenChange={() => onClose()}>
      <SheetContent side="right" className="w-full max-w-lg p-0 bg-card border-l border-border overflow-y-auto">
        {showAssignFlow ? (
          <BlockAssignCoachFlow
            block={block}
            coachProfile={coachProfile}
            onClose={onClose}
            onBack={() => setShowAssignFlow(false)}
          />
        ) : (
          <>
            {/* Header */}
            ...
            <div className="sticky bottom-0 p-4 bg-card border-t border-border space-y-2">
              {isOwned ? (
                <>
                  ...
                  <button onClick={() => setShowAssignFlow(true)} className="w-full py-3 rounded-xl border border-border font-display text-xs tracking-wider text-foreground hover:bg-secondary transition-colors flex items-center justify-center gap-2">
