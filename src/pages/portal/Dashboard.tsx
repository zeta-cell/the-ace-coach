...
interface CoachInfo { user_id: string; full_name: string; avatar_url: string | null; profile_slug: string | null; }
...
const Dashboard = () => {
  ...
  // Handle health OAuth callback
  const healthConnected = searchParams.get('health_connected');
  useEffect(() => {
    if (healthConnected) {
      toast.success(`${healthConnected.toUpperCase()} connected!`, { description: 'Your health data will sync automatically.' });
  ...
  const fetchGamificationData = async () => {
    ...
    const chartData = weeks.map(w => {
      const sessions = (plans || []).filter(p => { const d = new Date(p.plan_date); return d >= w.weekStart && d <= w.weekEnd; }).length;
      const xp = (xpEvents || []).filter(e => { const d = new Date(e.created_at); return d >= w.weekStart && d <= w.weekEnd; }).reduce((sum, e) => sum + e.xp_amount, 0);
    ...
    const { data: heatPlans } = await supabase.from("player_day_plans").select("plan_date").eq("player_id", user.id).gte("plan_date", threeMonthsAgo);
    const hmap: Record<string, number> = {};
    (heatPlans || []).forEach(p => { hmap[p.plan_date] = (hmap[p.plan_date] || 0) + 1; });
    setHeatmapData(hmap);
    ...
    // My coaches
    const { data: bookings } = await supabase.from("bookings").select("coach_id").eq("player_id", user.id).eq("status", "confirmed");
    const uniqueCoachIds = [...new Set((bookings || []).map(b => b.coach_id))];
    if (uniqueCoachIds.length > 0) {
      const [profilesRes, slugsRes] = await Promise.all([
        supabase.from("profiles").select("user_id, full_name, avatar_url").in("user_id", uniqueCoachIds.slice(0, 6)),
        supabase.from("coach_profiles").select("user_id, profile_slug").in("user_id", uniqueCoachIds.slice(0, 6)),
      ]);
      const slugMap = new Map((slugsRes.data || []).map(s => [s.user_id, s.profile_slug]));
      setMyCoaches((profilesRes.data || []).map(p => ({ ...p, profile_slug: slugMap.get(p.user_id) || null })));
    }
  ...
  const fetchPrograms = async () => {
    ...
    setPrograms(purchases.map(p => {
      const b = blockMap.get(p.block_id); const req = requestMap.get(p.block_id);
      return { ...p, title: b?.title || "Program", author_name: b?.author_name, author_avatar_url: b?.author_avatar_url, week_count: b?.week_count || 1, block_type: b?.block_type || "session", thumbnail_url: b?.thumbnail_url, coach_status: req ? (req.status === "accepted" ? "accepted" : "pending") as any : "none", coach_name: req ? coachNameMap.get(req.coach_id) || null : null, coach_id: req?.coach_id || null, request_id: req?.request_id || null };
    }));
  };
  ...
  return (
    <PortalLayout>
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
          <div className="bg-card border border-border rounded-xl p-3 text-center">
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              <p className="font-body text-[9px] text-muted-foreground uppercase">{lvl.label}</p>
            </div>
          </div>
          <div className="bg-card border border-border rounded-xl p-3 text-center">
        ...
        {/* Training Heatmap */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.17 }} className="bg-card border border-border rounded-xl p-5 mb-6">
          <h3 className="font-display text-sm tracking-wider text-muted-foreground mb-3">TRAINING HEATMAP</h3>
          <div className="flex flex-wrap gap-[2px]">
            {heatmapDays.map(d => {
              const key = format(d, "yyyy-MM-dd");
              const count = heatmapData[key] || 0;
              return <div key={key} className={`w-3 h-3 rounded-[2px] ${heatmapColor(count)}`} title={`${key}: ${count} sessions`} />;
            })}
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span className="text-[8px] font-body text-muted-foreground">Less</span>
            <div className="w-3 h-3 rounded-[2px] bg-secondary" />
            <div className="w-3 h-3 rounded-[2px] bg-green-300 dark:bg-green-800" />
            <div className="w-3 h-3 rounded-[2px] bg-green-500 dark:bg-green-600" />
            <div className="w-3 h-3 rounded-[2px] bg-green-700 dark:bg-green-400" />
            <span className="text-[8px] font-body text-muted-foreground">More</span>
          </div>
        </motion.div>

        {/* Health Data Card */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.19 }} className="mb-6">
          <HealthDataCard />
        </motion.div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {earnedBadges.slice(0, 8).map(b => (
                <div key={b.badge_key} className="shrink-0 w-20 bg-card border border-border rounded-xl p-3 text-center">
                  <div className="w-10 h-10 mx-auto rounded-full bg-primary/10 flex items-center justify-center mb-1"><Star size={16} className="text-primary" /></div>
        ...
        {/* My Coaches */}
        {myCoaches.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.27 }} className="mb-6">
            <h3 className="font-display text-sm tracking-wider text-muted-foreground mb-3">MY COACHES</h3>
            <div className="flex gap-2">
              {myCoaches.map(c => (
                <Link key={c.user_id} to={c.profile_slug ? `/coach/${c.profile_slug}` : "#"} className="shrink-0 text-center">
                  <div className="w-12 h-12 rounded-full bg-secondary overflow-hidden mx-auto mb-1 border-2 border-border hover:border-primary transition-colors">
                    {c.avatar_url ? <img src={c.avatar_url} alt="" className="w-full h-full object-cover" /> :
                      <div className="w-full h-full flex items-center justify-center font-display text-primary text-sm">{c.full_name?.charAt(0)}</div>}
                  </div>
                  <p className="font-body text-[9px] text-muted-foreground truncate w-14">{c.full_name?.split(" ")[0]}</p>
                </Link>
              ))}
            </div>
          </motion.div>
        ...
        {/* Health Connections */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.29 }} className="mb-6"><HealthConnections /></motion.div>

        {/* Raffle */}
        {isGoldPlus && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-6"><RaffleCard /></motion.div>}
